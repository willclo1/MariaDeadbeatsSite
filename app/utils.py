    ...
    for i, row_value in enumerate(left_column):
        for j, col_value in enumerate(top_row):
            ...
            if intersected_players:
                ...
                player_id = get_rarest_player(intersected_players, selected_player_ids)
                if player_id:
...
def get_rarest_player(player_ids, selected_player_ids):
    """
    Given a set of playerIDs, returns the playerID with the fewest total games played,
    ...
    try:
        ...
        print(f"Error in get_rarest_player(): {e}")
        return None
    finally:
        connection.close()
